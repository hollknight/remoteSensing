...
func UploadPicture(c *gin.Context) {
	...
	for i := 0; i < imgNum; i++ {
		localPath := fmt.Sprintf("%s%s.jpg", config.LocalPicPath, uuids[i])
		...
		fileName := fmt.Sprintf("project/%d/%d/%s.jpg", u.ID, projectID, uuids[i])
		...
		err = sql.AddPicture(global.GLOBAL_DB, uuids[i], names[i], uint64(projectID), math.MaxUint64, 0)
		if err != nil {
			common.Respond(c, errcode.ServerError, gin.H{})
			return
		}
